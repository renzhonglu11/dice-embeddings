...
class TorchTrainer(AbstractTrainer):
    ...
    def fit(self, *args, train_dataloaders, **kwargs) -> None:
        ...
        for epoch in range(self.attributes.max_epochs):
            ...
            print(
                f"Epoch:{epoch + 1} | Loss:{avg_epoch_loss:.8f} | Runtime:{(time.time() - start_time) / 60:.3f}mins"
            )
            ...
            if (
                self.attributes.auto_batch_finder
                and psutil.virtual_memory().percent < 30.0
                and counter < 5
            ):
                self.train_dataloaders = torch.utils.data.DataLoader(
                    dataset=self.train_dataloaders.dataset,
    ...
    def compute_forward_loss_backward(
        ...
        if self.use_closure:
            batch_loss = self.optimizer.step(
                closure=lambda: self.loss_function(self.model(x_batch), y_batch)
            )
            return batch_loss
        else:
