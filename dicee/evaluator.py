...
class Evaluator:
    ...
    def vocab_preparation(self, dataset) -> None:
        ...
        if isinstance(dataset.constraints, tuple):
            (
                self.domain_constraints_per_rel,
                self.range_constraints_per_rel,
            ) = dataset.constraints
        else:
            try:
                (
        ...
        pickle.dump(
            self.er_vocab, open(self.args.full_storage_path + "/er_vocab.p", "wb")
        )
        pickle.dump(
            self.re_vocab, open(self.args.full_storage_path + "/re_vocab.p", "wb")
        )
        pickle.dump(
            self.ee_vocab, open(self.args.full_storage_path + "/ee_vocab.p", "wb")
        )

    # @timeit
    def eval(
        self, dataset, trained_model, form_of_labelling, during_training=False
    ) -> None:
        ...
        elif self.args.scoring_technique in [
            "KvsAll",
            "KvsSample",
            "1vsAll",
            "PvsAll",
        ...
        if self.during_training is False:
            with open(
                self.args.full_storage_path + "/eval_report.json", "w"
            ) as file_descriptor:
    ...
    def dummy_eval(self, trained_model, form_of_labelling):
        if self.is_continual_training:
            self.er_vocab = pickle.load(
                open(self.args.full_storage_path + "/er_vocab.p", "rb")
            )
            self.re_vocab = pickle.load(
                open(self.args.full_storage_path + "/re_vocab.p", "rb")
            )
            self.ee_vocab = pickle.load(
                open(self.args.full_storage_path + "/ee_vocab.p", "rb")
            )
    ...
    def eval_rank_of_head_and_tail_entity(
        self, *, train_set, valid_set=None, test_set=None, trained_model
    ):
        ...
        if "train" in self.args.eval_model:
            res = self.evaluate_lp(
                trained_model, train_set, f"Evaluate {trained_model.name} on Train set"
            )
        ...
        if test_set is not None and "test" in self.args.eval_model:
            self.report["Test"] = self.evaluate_lp(
                trained_model, test_set, f"Evaluate {trained_model.name} of Test set"
            )
    ...
    def evaluate_lp_k_vs_all(
        self, model, triple_idx, info=None, form_of_labelling=None
    ):
        """
        ...
        if form_of_labelling == "RelationPrediction":
            ...
            for i in range(0, num_triples, self.args.batch_size):
                ...
                e1_idx_e2_idx, r_idx = torch.LongTensor(
                    data_batch[:, [0, 2]]
                ), torch.LongTensor(data_batch[:, 1])
            ...
            for i in range(0, num_triples, self.args.batch_size):
                ...
                e1_idx_r_idx, e2_idx = torch.LongTensor(
                    data_batch[:, [0, 1]]
                ), torch.tensor(data_batch[:, 2])
                ...
                for j in range(data_batch.shape[0]):
                    ...
                    if "constraint" in self.args.eval_model:
                        predictions[
                            j, self.range_constraints_per_rel[data_batch[j, 1]]
                        ] = -np.Inf
    ...
    def eval_with_data(
        self, dataset, trained_model, triple_idx: np.ndarray, form_of_labelling: str
    ):
        self.vocab_preparation(dataset)
