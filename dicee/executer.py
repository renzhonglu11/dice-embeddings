...
class Execute:
    ...
    def start(self) -> dict:
        ...
        #     wandb.init(
        #         project="dice_demo", config=config, name=f"{self.args.model}-{dataset}"
        #     )
        self.args = init_wandb(self.args)
...
class ParseDict(argparse.Action):
    def __call__(self, parser, namespace, values, option_string=None):
        setattr(
            namespace, self.dest, dict()
        )  # set each name of the attribute to hold the created object(s) as dictionary
        for value in values:
...
def get_default_arguments(description=None):
    ...
    parser.add_argument("--q", type=int, default=0, help="Q for Clifford Algebra")
    parser.add_argument(
        "--optim",
        type=str,
        default="Adam",
    ...
    parser.add_argument(
        ...
        type=str,
        default="None",
        help="[LayerNorm, BatchNorm1d, None]",
    )
    parser.add_argument(
        "--init_param", type=str, default=None, help="[xavier_normal, None]"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--kernel_size", type=int, default=3, help="Square kernel size for ConEx"
    )
    parser.add_argument(
