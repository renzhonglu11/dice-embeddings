from pykeen.contrib.lightning import LitModule, LCWALitModule
...
class MyLCWALitModule(LCWALitModule,Pykeen_Module):
    ...
    #     # path="/home/renzhong/graph_embedding/dice-embeddings/Experiments"+ "/test"
    ...
    #         entity_embedd = self.model.entity_representations[0].base[0].vocabulary._embeddings
    #         return (
    ...
    #             relation_embedd = self.model.base.relation_representations[0]._embeddings
    ...
    #         if isinstance(self.model.relation_representations[0],SingleCompGCNRepresentation):
    #             relation_embedd = self.model.relation_representations[0].combined.relation_representations._embeddings
    #         else:
    ...
    #         if isinstance(self.model.entity_representations[0],SingleCompGCNRepresentation):
    #             entity_embedd = self.model.entity_representations[0].combined.entity_representations._embeddings
    #         else:            
    #             entity_embedd = self.model.entity_representations[0]._embeddings

    #     return (
    #         entity_embedd.weight.data.detach() if entity_embedd != None else None,
    #         relation_embedd.weight.data.detach() if relation_embedd != None else None,
    #     )
    ...
    #     # if not torch.eq(x[0, 0], x[1, 0]):
    #     #     # head prediction
    #     #     # pred_score = predict.predict_target(
    #     #     #     model=self.model, tail=x[0, 2].item(), relation=relation_idx.item()
    #     #     # ).df["score"]
    #     #     pred_score = predict.predict_triples(
    #     #         model=self.model,
    #     #         triples = x,
    #     #     )

    #     # if not torch.eq(x[0, 2], x[1, 2]):
    #     #     # tail prediction
    #     #     pred_score = predict.predict_target(
    #     #         model=self.model, head=x[0, 0].item(), relation=relation_idx.item()
    #     #     ).df["score"]
    #     # pred_score_tensor = torch.from_numpy(np.array(pred_score.tolist()))
